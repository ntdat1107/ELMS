...
import {
    ...
    CREATE_CVS_REQUEST,
    CREATE_CVS_SUCCESS,
    GET_RECEIVE_CVS_FAIL,
    GET_RECEIVE_CVS_REQUEST,
    GET_RECEIVE_CVS_SUCCESS,
    GET_SEND_CVS_FAIL,
    GET_SEND_CVS_REQUEST,
    GET_SEND_CVS_SUCCESS,
...
export const createNewCvs = (subject, content, fastName) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.post(
            `/api/conversations/create/${fastName}`,
            { subject, content },
            config
        );
        dispatch({
...
export const getReceiveCvs = () => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.get(`/api/conversations/getreceive`, config);
        dispatch({
